...
import { Link2, Clock, CheckCircle, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
const columns: ColumnDef<IncapacidadPendiente>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (fallback) {
        return <p className="font-medium">{fallback.nombre}</p>;
      }
      return <span className="text-slate-400">—</span>;
    },
  },
    ...
    cell: ({ row }) => {
      ...
      return (
        <Button
          variant="default"
          size="sm"
          onClick={() => navigate(`/incapacidades/${row.original.id}/creacion-siniestro`)}
        >
          Crear Siniestro
        </Button>
      );
...
export function BandejaSiniestroPage() {
  const [filtros, setFiltros] = useState<FiltrosPendientes>({ skip: 0, limit: 100 });
  const [filtersOpen, setFiltersOpen] = useState(false);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ['incapacidades-bandeja-siniestro', filtros],
    queryFn: () => incapacidadService.listarBandejaCreacionSiniestro(filtros),
    staleTime: 30 * 1000,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Incapacidades ARL pendientes de creación de siniestro
          </p>
        </div>
          ...
          {!isLoading && total === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-xl font-medium text-slate-900">
                No hay incapacidades pendientes
              </h3>
              <p className="mt-2 text-slate-500">
                No hay incapacidades en estado CREACION_SINIESTRO.
              </p>
